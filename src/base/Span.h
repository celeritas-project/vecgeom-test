...
#define TCB_SPAN_NAMESPACE_NAME celeritas
...
class span {
 ...
 public:
  ...
  friend TCB_SPAN_CONSTEXPR iterator begin(span s) noexcept {
    return s.begin();
  }
...
template <typename ElementType, std::size_t Extent>
TCB_SPAN_CONSTEXPR span<ElementType, Extent> make_span(
...
TCB_SPAN_CONSTEXPR span<typename Container::value_type> make_span(
    Container& cont) {
  return {cont};
}
