 ...
 */
template <class T>
auto ToVector(span<T, 3> s) -> vecgeom::Vector3D<std::remove_cv_t<T>> {
  return {s[0], s[1], s[2]};
}
...
template <class T>
inline auto ToVector(const std::array<T, 3>& arr) -> vecgeom::Vector3D<T> {
  return ToVector(celeritas::make_span(arr));
}
